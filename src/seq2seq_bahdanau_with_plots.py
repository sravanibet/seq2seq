...
#Train
def main():
    TRAIN_PATH = "C:/Users/rashm/Downloads/eng-fra_train.tsv"
    TEST_PATH  = "C:/Users/rashm/Downloads/eng-fra_test.tsv"
    ...
    def eval_loss() -> float:
            ...
            total, n = 0.0, 0
            with torch.no_grad():
                for src, src_lens, tgt, _ in test_loader:
                    src = src.to(DEVICE)
                    src_lens = src_lens.to(DEVICE)
                    tgt = tgt.to(DEVICE)

                    out = model(src, src_lens, tgt, teacher_forcing=0.0)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        avg_train_loss = total / max(1, n)
        avg_test_loss = eval_loss()

        train_losses.append(avg_train_loss)
        test_losses.append(avg_test_loss)

        print(f"Epoch {ep:02d} DONE | train_loss={avg_train_loss:.4f} | test_loss={avg_test_loss:.4f}")
